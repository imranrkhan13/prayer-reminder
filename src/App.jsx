...
import {
  ...
  Download, Check, Palette, Bell, VolumeX,
...
const HADITHS_ALERT = [
  ...
  'O servant of Allah — the angels are calling you right now. Come to prayer. Come to success. The Prophet ﷺ said whoever prays Fajr is under Allah\'s protection all day. Stand now, make wudu, and answer your Lord\'s call.',
];
const DUAS = [
...
function Particles({ theme }) {
  const pts = useMemo(() => Array.from({ length: 8 }, (_, i) => ({ id: i, x: Math.random() * 100, y: Math.random() * 100, size: Math.random() * 5 + 2, dur: Math.random() * 7 + 5, delay: Math.random() * 3 })), []);
  return (
...
// ═══════════════════════════════════════════════════════════════════════════════
// FIRST-LAUNCH TOUR  (shown ONCE, never again)
// ═══════════════════════════════════════════════════════════════════════════════
const TOUR = [
  {
    emoji: '🕌', title: 'Welcome to NUR', sub: 'نور means "Light" in Arabic',
    body: 'NUR is your personal prayer companion. It shows you prayer times for your exact city, reminds you before each prayer, reads Islamic wisdom aloud, and tracks which prayers you\'ve completed.',
    cta: 'Let\'s Go →'
  },
  {
    emoji: '📍', title: 'Real Prayer Times', sub: 'From the Aladhan API — accurate to your city',
    body: 'Prayer times differ by city. Fajr in Mumbai is not the same as Fajr in London. NUR fetches accurate times daily using the free Aladhan.com prayer API based on your GPS or city name.',
    cta: 'Got it →'
  },
  {
    emoji: '🔔', title: 'Lock-Screen Alerts', sub: 'Never miss a prayer again',
    body: 'NUR sends you a full-screen alert when it\'s prayer time — even when your phone is locked. It also warns you 30 and 15 minutes before a prayer window closes, so you never run out of time.',
    cta: 'Allow Notifications →', action: 'notif'
  },
  {
    emoji: '🕌', title: 'Mosque Mode', sub: 'One tap silences everything at the masjid',
    body: 'If you\'re heading to the mosque, tap 🔔 in the footer to enable Mosque Mode. This silences all sounds and notifications so your phone doesn\'t disturb anyone. NUR re-enables after 2 hours automatically.',
    cta: 'Understood →'
  },
  {
    emoji: '📊', title: 'Prayer Streak', sub: 'Track your consistency',
    body: 'Every prayer you confirm gets logged. The 🔥 streak shows how many days in a row you prayed all 5 prayers. Tap the streak tile or "Prayer Tracker" button to see your full history chart.',
    cta: 'Nice →'
  },
  {
    emoji: '🧭', title: 'Calculation Method', sub: 'Which Islamic authority covers your region?',
    body: 'Different Islamic organisations calculate prayer times slightly differently. Choose the one used in your country. If you\'re not sure, MWL (Muslim World League) works for most of the world.',
    cta: 'Choose & Continue →', action: 'method'
  },
];
...
function FirstLaunchTour({ theme, onComplete }) {
  ...
  const next = async () => {
    if (slide.action === 'notif') {
      const ok = await askNotifPermission();
      setNotifOk(ok);
    }
    if (isLast) { onComplete(method); }
    else { setStep(s => s + 1); }
  };
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-8 overflow-y-auto pb-6">
        <AnimatePresence mode="wait">
          <motion.div key={step}
            ...
            <motion.div className="text-7xl mb-5" animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2.5, repeat: Infinity }}>
              {slide.emoji}
            </motion.div>
            ...
            {slide.action === 'notif' && notifOk && (
              <div className="flex items-center gap-2 w-full px-4 py-3 rounded-2xl mb-4" style={{ background: `${theme.accent}15` }}>
                ...
                <p className="text-xs font-black" style={{ color: theme.accent }}>Notifications are ON ✓</p>
              </div>
            )}
            {slide.action === 'notif' && !notifOk && Notification?.permission === 'denied' && (
              <div className="flex items-start gap-2 w-full px-4 py-3 rounded-2xl mb-4" style={{ background: `rgba(224,92,122,0.1)` }}>
                ...
                <p className="text-xs font-bold" style={{ color: '#e05c7a' }}>
                  Notifications are blocked. Go to your phone's Settings → NUR → Notifications to enable them.
                </p>
              </div>
            )}
                ...
                {CALC_METHODS.map(m => (
                  <motion.button key={m.id} whileTap={{ scale: 0.97 }}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-black text-sm" style={{ color: theme.text }}>{m.name}</p>
                        <p className="text-[9px] mt-0.5" style={{ color: theme.muted }}>{m.region}</p>
                      </div>
            ...
            {step > 0 && (
              <button onClick={() => setStep(s => s - 1)} className="mt-4 text-xs font-bold" style={{ color: theme.muted }}>
                ← Back
              </button>
            )}
          </motion.div>
...
function LocationSetup({ theme, method, onDone }) {
  const [phase, setPhase] = useState('idle'); // idle | locating | fetching | error
  ...
  const doCoords = async (lat, lng) => {
    setPhase('fetching');
    try {
      const times = await fetchByCoords(lat, lng, method);
      onDone({ lat, lng, times, source: 'gps' });
    } catch {
      setPhase('error');
      setErrMsg('Could not load prayer times. Please check your internet connection and try again.');
    }
  };

  const useGPS = () => {
    setPhase('locating');
    if (!navigator.geolocation) { setPhase('error'); setErrMsg('Your browser doesn\'t support GPS. Enter your city name below.'); setShowManual(true); return; }
    navigator.geolocation.getCurrentPosition(
      p => doCoords(p.coords.latitude, p.coords.longitude),
      () => { setPhase('error'); setErrMsg('Location access was denied. No problem — enter your city name below instead.'); setShowManual(true); },
      { timeout: 10000 }
    );
  };
  ...
  const doCity = async () => {
    ...
    setPhase('fetching');
    try {
      const times = await fetchByCity(city, country, method);
      onDone({ city, country, times, source: 'city' });
    } catch {
      setPhase('error');
      setErrMsg(`Couldn't find "${city}". Try a nearby major city name in English (e.g. "Mumbai" not "Bombay").`);
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div className="relative z-10 w-full max-w-sm flex flex-col items-center text-center">
        ...
        <h2 className="text-3xl font-black tracking-tighter mb-2" style={{ color: theme.text }}>
          {spinning ? 'Please wait…' : 'Your City'}
        </h2>
        <p className="text-sm leading-relaxed mb-5" style={{ color: theme.muted }}>
          {phase === 'idle' ? 'NUR needs to know your city to show the correct prayer times. Your location is never stored or shared with anyone.' :
            phase === 'locating' ? 'Finding your location…' :
              phase === 'fetching' ? 'Downloading today\'s prayer times from Aladhan API…' :
                errMsg}
        </p>
        ...
        {spinning && (
          <div className="flex gap-2">
            {[0, 1, 2].map(i => (
              <motion.div key={i} className="w-2 h-2 rounded-full" style={{ background: theme.accent }}
        ...
        {(phase === 'idle' || phase === 'error') && !showManual && (
          <div className="w-full space-y-3">
            ...
            <p className="text-xs" style={{ color: theme.muted }}>
              This asks your device for your GPS coordinates. Fastest & most accurate.
            </p>
            <button onClick={() => setShowManual(true)} className="text-sm font-bold underline" style={{ color: theme.muted }}>
              I'd rather type my city name
            </button>
          </div>
        )}
        ...
        {showManual && (
          <div className="w-full space-y-3">
            <div className="text-left">
              <p className="text-[10px] font-black uppercase mb-1" style={{ color: theme.muted }}>City Name *</p>
              <input value={city} onChange={e => setCity(e.target.value)}
                placeholder="e.g. Mumbai, London, New York"
                className="w-full p-4 rounded-2xl font-bold text-sm outline-none border-2 bg-transparent"
                style={{ borderColor: `${theme.accent}40`, color: theme.text }}
                onKeyDown={e => e.key === 'Enter' && doCity()} />
            </div>
            <div className="text-left">
              <p className="text-[10px] font-black uppercase mb-1" style={{ color: theme.muted }}>Country (optional but helps accuracy)</p>
            ...
            <button onClick={useGPS} className="text-xs font-bold" style={{ color: theme.muted }}>
              Try GPS instead
            </button>
          </div>
        )}
  ...
  return (
    <motion.div initial={{ y: -40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -40, opacity: 0 }}
      ...
      <div className="flex items-center gap-2">
        <span>🕌</span>
        <div>
          <p className="text-[10px] font-black text-white">Mosque Mode is ON</p>
          <p className="text-[8px] text-white/60">All sounds & notifications are silenced</p>
        </div>
      </div>
      <motion.button whileTap={{ scale: 0.9 }} onClick={onDisable}
        className="px-3 py-1.5 rounded-xl text-[9px] font-black text-white border border-white/30">
        Turn Off
      </motion.button>
    </motion.div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 60 }}
      ...
      <div className="relative w-full max-w-lg mx-auto rounded-t-[2.5rem] p-8 pb-12" style={{ background: theme.surface }}>
        <div className="flex items-start gap-4 mb-4">
          ...
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest mb-0.5" style={{ color: theme.accent }}>
              30 minutes until {prayer.name}
            </p>
            <h3 className="text-xl font-black" style={{ color: theme.text }}>Are you going to the mosque?</h3>
            <p className="text-xs mt-1 leading-relaxed" style={{ color: theme.muted }}>
              If yes, we'll silence all sounds and notifications — so your phone doesn't disturb anyone at the masjid. NUR turns back on automatically after 2 hours.
            </p>
          </div>
        </div>
        <div className="flex flex-col gap-2 mt-4">
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.95 }} onClick={onYes}
            ...
            style={{ background: 'linear-gradient(135deg,#1a7a4a,#0d5234)', boxShadow: '0 6px 20px rgba(26,122,74,0.3)' }}>
            🕌 Yes — Enable Mosque Mode
          </motion.button>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }} onClick={onNo}
            className="w-full py-4 rounded-[1.5rem] font-black"
            style={{ background: theme.overlay, color: theme.text }}>
            🏠 No, Praying at Home
          </motion.button>
          <button onClick={onDismiss} className="text-xs font-bold py-2 text-center" style={{ color: theme.muted }}>
...
function PrayerAlert({ prayer, theme, onPraying, onDismiss, silenced, endTime }) {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setCd(c => {
        if (c <= 1) {
          clearInterval(t);
          if (!fired.current && !silenced) {
            fired.current = true;
            speak([`It is time for ${prayer.name} prayer. `, HADITHS_ALERT[0] + ' ', HADITHS_ALERT[1] + ' ', HADITHS_ALERT[2]].join(''), 0.85);
          }
          return 0;
        }
        return c - 1;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <Particles theme={theme} />
      {silenced && (
        <div className="absolute top-6 right-6 flex items-center gap-1.5 px-3 py-2 rounded-2xl" style={{ background: theme.overlay }}>
          <VolumeX size={12} style={{ color: theme.muted }} /><span className="text-[9px] font-black" style={{ color: theme.muted }}>Mosque Mode</span>
        </div>
      )}
      <div className="relative mb-8">
        {[1, 2, 3].map(i => (
          <motion.div key={i} className="absolute rounded-full border-2"
            style={{ borderColor: theme.accent, inset: -(i * 22), opacity: 0.12 }}
            animate={{ scale: [1, 1.18, 1], opacity: [0.12, 0.03, 0.12] }}
      ...
      {cd > 0 ? (
        <div className="flex flex-col items-center mb-6">
          <p className="text-xs mb-2" style={{ color: theme.muted }}>{silenced ? 'Mosque Mode — Sound Off' : 'Voice reminder starts in'}</p>
          <motion.p className="text-5xl font-black" style={{ color: theme.accent }}
            animate={{ scale: [1, 1.25, 1] }} transition={{ duration: 1, repeat: Infinity }}>{cd}</motion.p>
        </div>
      ) : (
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm mb-6 flex items-center gap-2" style={{ color: theme.accent }}>
          {silenced ? <><VolumeX size={14} /> Mosque Mode — Audio Silenced</> : <><motion.span animate={{ scale: [1, 1.5, 1] }} transition={{ duration: 0.7, repeat: Infinity }}>🔊</motion.span> Reading 3 hadiths aloud…</>}
        </motion.p>
      )}
      <div className="w-full max-w-xs px-6 flex flex-col gap-3">
        <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.94 }} onClick={onPraying}
          ...
          style={{ background: theme.gradient, boxShadow: `0 8px 30px ${theme.glow}` }}>
          <Check size={22} /> I Am Praying Now
        </motion.button>
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }} onClick={onDismiss}
          className="w-full py-4 rounded-[2rem] font-bold text-sm"
          style={{ background: theme.overlay, color: theme.muted }}>
          Dismiss (I'll pray later)
        </motion.button>
      </div>
    </motion.div>
  );
...
function DuaScreen({ prayer, theme, onClose, silenced }) {
  ...
  if (phase === 'done') return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 280, damping: 18 }}
        ...
        <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }} onClick={onClose}
          className="px-10 py-4 rounded-[2rem] font-black text-white"
          style={{ background: theme.gradient, boxShadow: `0 8px 30px ${theme.glow}` }}>Ameen 🤲</motion.button>
      </motion.div>
    </motion.div>
  );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 30 }}
      ...
      style={{ background: theme.bg, fontFamily: 'Poppins, sans-serif' }}>
      <Particles theme={theme} />
      <motion.div key={phase} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        className="flex flex-col items-center text-center max-w-sm relative z-10">
        <motion.div className="text-5xl mb-4" animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 2.5, repeat: Infinity }}>
          {phase === 'dua' ? '🤲' : '✨'}
        </motion.div>
        <p className="text-[9px] font-black uppercase tracking-[0.4em] mb-1" style={{ color: theme.accent }}>
          {phase === 'dua' ? 'Post-Prayer Dhikr' : 'Why Prayer Matters'}
        </p>
        <p className="text-[10px] mb-3" style={{ color: theme.muted }}>
          {phase === 'dua' ? 'Recite these after your prayer to multiply your reward' : 'A reminder of the power of your prayer'}
        </p>
        <div className="p-5 rounded-[1.5rem] mb-4 text-left" style={{ background: theme.surface }}>
          <p className="text-sm font-bold leading-relaxed" style={{ color: theme.text }}>
            {phase === 'dua' ? DUAS[di] : VIRTUES[vi]}
          </p>
        </div>
        <div className="flex items-center gap-3 mb-4">
          {!silenced && (
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.88 }}
              onClick={() => speak(phase === 'dua' ? DUAS[di] : VIRTUES[vi])}
              className="flex items-center gap-1.5 px-4 py-2 rounded-2xl text-xs font-black"
              style={{ background: theme.overlay, color: theme.accent }}>
              <Volume2 size={13} /> Read aloud
            </motion.button>
          )}
        </div>
        <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.94 }}
          onClick={() => setPhase(phase === 'dua' ? 'virtue' : 'done')}
          className="w-full py-5 rounded-[2rem] font-black text-white text-base"
          style={{ background: theme.gradient, boxShadow: `0 8px 30px ${theme.glow}` }}>
          {phase === 'dua' ? 'Next: Why Prayer Matters →' : 'Done ✓'}
        </motion.button>
      </motion.div>
    </motion.div>
  );
...
function FollowUp({ prayer, theme, onYes, onNo }) {
  return (
    <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 60 }}
      className="fixed inset-0 z-[450] flex items-end justify-center" style={{ fontFamily: 'Poppins, sans-serif' }}>
      <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={onNo} />
      <div className="relative w-full max-w-lg mx-auto rounded-t-[2.5rem] p-8 pb-12" style={{ background: theme.surface }}>
        <div className="flex items-center gap-4 mb-3">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl shrink-0"
            style={{ background: theme.gradient }}>{PRAYER_META[prayer.name]?.icon || '🕌'}</div>
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest mb-0.5" style={{ color: theme.accent }}>
              Quick check • 15 min ago
            </p>
        ...
        <div className="flex gap-3 mt-5">
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.94 }} onClick={onYes}
            className="flex-1 py-4 rounded-[1.5rem] font-black text-white flex items-center justify-center gap-2"
            style={{ background: theme.gradient, boxShadow: `0 6px 20px ${theme.glow}` }}>
            <Check size={18} /> Yes, Alhamdulillah!
          </motion.button>
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.94 }} onClick={onNo}
            className="flex-1 py-4 rounded-[1.5rem] font-black"
            style={{ background: theme.overlay, color: theme.muted }}>Not yet</motion.button>
        </div>
      </div>
    </motion.div>
  );
...
function Tracker({ prayerLog, theme, onClose }) {
  ...
  const last7 = Array.from({ length: 7 }, (_, i) => {
    ...
    return { label: d.toLocaleDateString('en', { weekday: 'short' }), count: Object.values(prayerLog[key] || {}).filter(Boolean).length, key };
  });
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[300] flex flex-col" style={{ background: theme.bg, fontFamily: 'Poppins, sans-serif' }}>
      <Particles theme={theme} />
      <div className="relative z-10 p-7 flex justify-between items-center shrink-0">
        <div>
          <h3 className="text-3xl font-black tracking-tighter" style={{ color: theme.text }}>Prayer Tracker</h3>
          <p className="text-xs font-bold" style={{ color: theme.muted }}>Your journey — every prayer counts</p>
        </div>
        <motion.button whileTap={{ scale: 0.88, rotate: 90 }} onClick={onClose}
          className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: theme.overlay }}>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-6 pb-10 space-y-4">
        <div className="grid grid-cols-3 gap-3">
          {[
            { icon: '🔥', val: full, label: 'Perfect Days', tip: 'Days you prayed all 5 prayers' },
            { icon: '📅', val: Object.keys(prayerLog).length, label: 'Days Tracked', tip: 'How long NUR has been tracking you' },
            { icon: '⭐', val: total, label: 'Total Prayers', tip: 'Every individual prayer you\'ve logged' },
          ].map(s => (
            <motion.div key={s.label} whileHover={{ scale: 1.06, y: -3 }}
              className="rounded-[1.5rem] p-4 flex flex-col items-center" style={{ background: theme.surface }}>
            ...
            {last7.map(d => (
              <div key={d.key} className="flex flex-col items-center flex-1 gap-1 h-full justify-end">
                <motion.div initial={{ height: 0 }}
                  animate={{ height: `${Math.max((d.count / 5) * 70, d.count > 0 ? 8 : 4)}px` }}
                  transition={{ type: 'spring', stiffness: 200 }}
                  className="w-full rounded-t-xl"
                  style={{ background: d.count === 5 ? theme.gradient : theme.accent, opacity: d.count === 0 ? 0.15 : 1 }} />
        ...
        <div className="rounded-[2rem] p-5" style={{ background: theme.surface }}>
          ...
          <div className="space-y-2">
            {ACTIVE_PRAYERS.map(name => {
              const prayed = todayLog[name];
              const meta = PRAYER_META[name];
              return (
                <motion.div key={name} whileHover={{ x: 4 }}
                  className="flex items-center justify-between py-2.5 border-b" style={{ borderColor: `${theme.accent}12` }}>
                  ...
                  <AnimatePresence mode="wait">
                    {prayed ? (
                      <motion.div key="y" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 400 }}
                        className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: theme.gradient }}>
            ...
            <div className="flex items-center gap-2">
              <div className="w-24 h-2 rounded-full overflow-hidden" style={{ background: `${theme.accent}20` }}>
                <motion.div initial={{ width: 0 }}
                  animate={{ width: `${(Object.values(todayLog).filter(Boolean).length / 5) * 100}%` }}
                  transition={{ duration: 0.8, type: 'spring' }}
                  className="h-full rounded-full" style={{ background: theme.gradient }} />
              </div>
              <span className="text-xs font-black" style={{ color: theme.accent }}>
                {Object.values(todayLog).filter(Boolean).length}/5
              </span>
            </div>
          </div>
        </div>
...
function QuranReader({ prayer, theme, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.alquran.cloud/v1/surah/${meta.surahId}/editions/quran-uthmani,en.sahih`)
      .then(r => r.json())
      .then(d => {
        setData(d.data[0].ayahs.map((a, i) => ({ arabic: a.text, english: d.data[1].ayahs[i].text, n: a.numberInSurah })));
        setLoading(false);
  ...
  return (
    <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
      ...
      <div className="p-7 flex justify-between items-center shrink-0 border-b" style={{ borderColor: `${theme.accent}18` }}>
        <div>
          <p className="text-[9px] font-black uppercase tracking-widest mb-1" style={{ color: theme.accent }}>
            Recommended Surah for {prayer.name}
          </p>
          <h3 className="text-3xl font-black tracking-tighter" style={{ color: theme.text }}>{meta?.surahName}</h3>
          <p className="text-[10px] mt-0.5" style={{ color: theme.muted }}>Arabic above · English translation below each verse</p>
        </div>
        <motion.button whileTap={{ scale: 0.88, rotate: 90 }} onClick={onClose}
          className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: theme.overlay }}>
          <X size={20} style={{ color: theme.text }} />
        ...
        {loading ? (
          <div className="flex justify-center items-center h-40">
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
              className="w-8 h-8 rounded-full border-2 border-t-transparent" style={{ borderColor: theme.accent }} />
          </div>
        ) : data.map((a, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            transition={{ delay: Math.min(i * 0.03, 0.5) }}
            className="mb-10 pb-8 border-b" style={{ borderColor: `${theme.accent}12` }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center text-[10px] font-black text-white"
                style={{ background: theme.gradient }}>{a.n}</div>
              <p className="text-[9px]" style={{ color: theme.muted }}>Verse {a.n}</p>
            </div>
            <p className="text-5xl text-right font-bold leading-[2.6] mb-5 select-text" dir="rtl"
              style={{ color: theme.accent, fontFamily: "'Scheherazade New','Amiri',serif" }}>
              {a.arabic}
            </p>
...
function ThemePicker({ current, onChange, onClose, theme }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[300] flex items-end" style={{ fontFamily: 'Poppins, sans-serif' }}>
      ...
      <div className="relative w-full max-w-lg mx-auto rounded-t-[2.5rem] p-7 pb-12 shadow-2xl" style={{ background: theme.surface }}>
        <div className="flex justify-between items-start mb-1">
          <div>
            <p className="font-black text-sm uppercase tracking-widest" style={{ color: theme.muted }}>App Theme</p>
            <p className="text-xs" style={{ color: theme.muted }}>Choose the colour style you prefer</p>
          </div>
          <motion.button whileTap={{ scale: 0.88 }} onClick={onClose}
            className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: theme.overlay }}>
            <X size={15} style={{ color: theme.text }} />
          </motion.button>
        </div>
        <div className="grid grid-cols-5 gap-4 mt-5">
          {Object.entries(THEMES).map(([key, t]) => (
            <motion.button key={key} whileHover={{ scale: 1.14, y: -5 }} whileTap={{ scale: 0.9 }}
              onClick={() => { onChange(key); onClose(); }}
              className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 rounded-2xl"
                style={{
                  background: t.gradient,
                  boxShadow: current === key ? `0 4px 18px ${t.glow}` : 'none',
                  outline: current === key ? `3px solid ${t.accent}` : '3px solid transparent',
...
export default function App() {
  ...
  // Auto-refresh times if it's a new day
  useEffect(() => {
    const today = new Date().toDateString();
  ...
  const doRefresh = async (loc) => {
    ...
    try {
      ...
      const today = new Date().toDateString();
      setTimesDate(today);
      LS.set('nur_times', times);
      LS.set('nur_times_date', today);
  ...
  const prayerList = useMemo(() => {
    ...
    return ACTIVE_PRAYERS.map((name, i) => ({
      name, icon: PRAYER_META[name].icon,
      time: prayerTimes[name] || '--:--',
      endTime: prayerTimes[ACTIVE_PRAYERS[(i + 1) % ACTIVE_PRAYERS.length]] || '--:--',
      surahId: PRAYER_META[name].surahId,
      surahName: PRAYER_META[name].surahName,
    }));
  }, [prayerTimes]);

  // Upcoming/current prayer
  const { upcoming, current, minsUntil } = useMemo(() => {
    ...
    for (const p of prayerList) {
      ...
      if (e > s) { if (now >= s && now < e) { cur = p; break; } }
      else { if (now >= s || now < e) { cur = p; break; } }
    }
    let diff = timeToMins(up.time) - now;
    if (diff < 0) diff += 1440;
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      prayerList.forEach(p => {
        // Prayer start alert
        if (p.time === now && !fired.current.has(`s-${now}-${p.name}`)) {
          fired.current.add(`s-${now}-${p.name}`);
          setTimeout(() => fired.current.delete(`s-${now}-${p.name}`), 120000);
          if (!silenced) pushNotif(`🕌 ${p.name} Time`, `Window: ${p.time} – ${p.endTime}`, `start-${p.name}`);
          setPrayerAlert(p);
        }
        // 30 min before start → mosque prompt
        const m30 = minsBefore(p.time, 30);
        if (m30 === now && !fired.current.has(`mp-${now}-${p.name}`)) {
          fired.current.add(`mp-${now}-${p.name}`);
          setTimeout(() => fired.current.delete(`mp-${now}-${p.name}`), 120000);
          if (!silenced) setMosquePrompt(p);
        }
        // 30 min before end
        const e30 = minsBefore(p.endTime, 30);
        if (e30 === now && !fired.current.has(`e30-${now}-${p.name}`)) {
          fired.current.add(`e30-${now}-${p.name}`);
          setTimeout(() => fired.current.delete(`e30-${now}-${p.name}`), 120000);
          if (!silenced) pushNotif(`⏰ ${p.name} — 30 min left`, `Window closes at ${p.endTime}. Don't miss it!`, `e30-${p.name}`);
        }
        // 15 min before end
        const e15 = minsBefore(p.endTime, 15);
        if (e15 === now && !fired.current.has(`e15-${now}-${p.name}`)) {
          fired.current.add(`e15-${now}-${p.name}`);
          setTimeout(() => fired.current.delete(`e15-${now}-${p.name}`), 120000);
          if (!silenced) pushNotif(`⚠️ ${p.name} ending in 15 min!`, `Last chance — window closes at ${p.endTime}`, `e15-${p.name}`);
        }
      });
    };
    check();
  ...
  const scheduleFU = useCallback((p) => {
    ...
    fuTimers.current[p.name] = setTimeout(() => {
      ...
      if (!log[new Date().toDateString()]?.[p.name]) {
        if (!silenced) pushNotif(`Did you pray ${p.name}? 🤲`, 'Tap to log your prayer', `fu-${p.name}`);
        setFollowUp(p);
      }
  ...
  const enableMosque = () => {
    const u = Date.now() + 2 * 60 * 60 * 1000;
    setMosqueModeUntil(u); LS.set('nur_mosque_until', u);
    setMosquePrompt(null); window.speechSynthesis?.cancel();
  };
  ...
  const { streak, todayCount } = useMemo(() => {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      if (Object.values(prayerLog[d.toDateString()] || {}).filter(Boolean).length === 5) s++;
      else if (i > 0) break;
    }
  ...
  return (
    <>
      ...
      <div className="h-screen w-full max-w-lg mx-auto flex flex-col overflow-hidden relative"
        ...
        <AnimatePresence>
          {silenced && <MosqueModeBanner key="mb" theme={theme} onDisable={() => setMosqueModeUntil(0)} />}
        </AnimatePresence>
        ...
        <div className="relative z-10 mx-5 mb-3 shrink-0">
          {!prayerTimes ? (
            <motion.div whileHover={{ scale: 1.01 }} onClick={() => setShowLocSetup(true)}
              ...
              <div>
                <p className="text-white/70 text-[9px] font-black uppercase tracking-widest">Setup Needed</p>
                <p className="text-white font-black text-lg">Tap to enter your city</p>
                <p className="text-white/60 text-xs">We need your city to show prayer times</p>
              </div>
            </motion.div>
          ) : timesError ? (
            <motion.div className="rounded-[2rem] p-5 flex items-center gap-4"
              style={{ background: 'linear-gradient(135deg,#e05c7a,#b83d5c)' }}>
              <AlertCircle size={28} color="white" />
              <div>
                <p className="text-white font-black">Couldn't load prayer times</p>
                <p className="text-white/70 text-xs">Check your internet and tap 🔄 in the header</p>
              </div>
            </motion.div>
          ) : loadingTimes ? (
            ...
            <motion.div whileHover={{ scale: 1.01 }} className="rounded-[2rem] p-5 relative overflow-hidden"
              style={{ background: theme.gradient, boxShadow: `0 8px 30px ${theme.glow}` }}>
              <div className="absolute right-0 top-0 w-36 h-36 rounded-full bg-white/10" style={{ transform: 'translate(30%,-30%)' }} />
              <div className="flex justify-between items-start relative z-10">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-[0.4em] text-white/60 mb-1">
                    {current ? '🟢 Active Now' : '⏳ Next Prayer'}
                  </p>
                  <h2 className="text-4xl font-black tracking-tighter text-white">
                    {(current || upcoming)?.name}
                  </h2>
                  <p className="text-white/80 font-bold text-sm mt-1">
                    {current ? `${current.time} — closes ${current.endTime}` : `Starts in ${formatMins(minsUntil)} · at ${upcoming?.time}`}
                  </p>
                ...
                <div className="relative z-10 mt-3 flex items-center gap-2">
                  <div className="flex-1 h-1.5 rounded-full bg-white/20">
                    <motion.div className="h-full rounded-full bg-white/80"
                      animate={{ width: `${Math.min(Math.max(100 - (minsUntil / 120) * 100, 2), 100)}%` }}
        ...
        {/* BENTO GRID */}
        <div className="relative z-10 flex-1 grid grid-cols-6 gap-3 px-5 pb-2 overflow-hidden"
          ...
          <motion.div whileHover={{ scale: 1.015 }}
            ...
            <div className="absolute right-0 top-0 w-48 h-48 rounded-full"
              style={{ background: theme.gradient, opacity: 0.05, transform: 'translate(35%,-35%)' }} />
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest" style={{ color: theme.accent }}>
                Today's Hadith #{hadithIdx + 1}
              </span>
              <Tip theme={theme} text="A different saying of the Prophet ﷺ is shown here every day. Tap the 🔊 button to hear it read aloud in English." />
            </div>
            <p className="text-[13px] font-black leading-tight italic mb-2 px-2" style={{ color: theme.text }}>
              "{hadith.en}"
            </p>
            <div className="flex items-center gap-2 mb-2">
            ...
            <motion.button whileTap={{ scale: 0.9 }}
              ...
              {silenced ? <VolumeX size={10} /> : <Volume2 size={10} />}
              {silenced ? 'Silenced' : 'Hear aloud'}
            </motion.button>
          </motion.div>

          {/* PRAYER TILES */}
          {prayerList.map((prayer, i) => {
            const prayed = todayLog[prayer.name];
            const isWide = i >= 3;
            return (
              <motion.div key={prayer.name} whileHover={{ y: -5, scale: 1.03 }} whileTap={{ scale: 0.93 }}
                ...
                title={`Tap to read ${prayer.surahName}`}>
                <motion.div className="absolute inset-0 rounded-[2rem]" initial={{ opacity: 0 }} whileHover={{ opacity: 1 }}
                  style={{ background: theme.gradient }} transition={{ duration: 0.18 }} />
                {prayed && (
                ...
                <div className="relative z-10 flex justify-between items-start">
                  <span className="text-[9px] font-black uppercase opacity-50">{prayer.name}</span>
                  <span className="text-base">{prayer.icon}</span>
                </div>
                <div className="relative z-10">
                  <p className="text-xl font-black tracking-tighter">{prayer.time}</p>
                  <p className="text-[8px] font-bold opacity-40 mt-0.5 flex items-center gap-1">
                    <BookOpen size={7} /> {prayer.surahName}
                  </p>
                </div>
              </motion.div>
            );
          ...
          <motion.div whileHover={{ scale: 1.04 }}
            className="col-span-3 row-span-1 rounded-[1.5rem] flex items-center gap-2 px-4"
            style={{ background: theme.overlay }}
            title="Switch the language of the daily hadith">
            <Globe size={12} style={{ color: theme.accent }} />
            <select value="en" className="bg-transparent font-black text-[10px] uppercase outline-none w-full cursor-pointer" style={{ color: theme.text }}>
          ...
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            ...
            style={{ background: theme.surface }}
            title="Tap to see your full prayer history and streak chart">
            ...
            <div>
              <p className="text-[10px] font-black leading-tight" style={{ color: theme.accent }}>{streak} Day Streak</p>
              <p className="text-[8px] font-bold" style={{ color: theme.muted }}>{todayCount}/5 today · tap to see</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="relative z-10 px-5 pb-7 pt-2 shrink-0 flex gap-3">
          <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.9 }}
            ...
            title={silenced ? "Mosque Mode is ON — tap to turn off sounds" : "Tap to enable Mosque Mode — silences everything for 2 hours"}
            ...
            <span className="text-[7px] font-black" style={{ color: silenced ? '#fff' : theme.muted }}>
              {silenced ? 'Mosque' : 'Alerts'}
            </span>
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.02, y: -2, boxShadow: `0 20px 40px ${theme.glow}80` }}
            ...
            <motion.div initial={{ x: '-100%', skewX: -45 }} whileHover={{ x: '200%' }}
      ...
      <AnimatePresence>
        {showTour && (
          <FirstLaunchTour key="tour" theme={theme} onComplete={method => {
            setCalcMethod(method); LS.set('nur_method', method);
            LS.set('nur_onboarded', true); setShowTour(false); setShowLocSetup(true);
          }} />
        )}
        {showLocSetup && (
          <LocationSetup key="loc" theme={theme} method={calcMethod} onDone={loc => {
            setLocationData(loc); LS.set('nur_location', loc);
            setPrayerTimes(loc.times); LS.set('nur_times', loc.times);
            LS.set('nur_times_date', new Date().toDateString());
            setShowLocSetup(false);
          }} />
        )}
        ...
        {mosquePrompt && !silenced && (
          <MosquePrompt key="mp" prayer={mosquePrompt} theme={theme}
            onYes={enableMosque} onNo={() => setMosquePrompt(null)} onDismiss={() => setMosquePrompt(null)} />
        )}
        {prayerAlert && (
          <PrayerAlert key="pa" prayer={prayerAlert} theme={theme}
            silenced={silenced} endTime={prayerAlert.endTime}
        ...
        {followUp && (
          <FollowUp key="fu" prayer={followUp} theme={theme}
